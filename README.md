...
只用名字（first names）的结果：

| 数据集 | 训练集 | 训练集 | 验证集 | 验证集 | 测试集 | 测试集 |
| :---: | :---: | :---: |:---: | :---: | :---: | :---: |
| 是否包括未知性别 | 是 | 否 | 是 | 否 | 是 | 否 |
...
显然，只用名字的训练效果要稍微高于用全名的，尽管效果不是很明显，跟上述朴素贝叶斯和逻辑回归不一样，详见之前做的[evaluation](https://github.com/jaaack-wang/gender-predicator/tree/main/evaluation)。另外，这里训练的四个模型显然效果要好于之前的素贝叶斯和逻辑回归模型。除了LR，就算是用全名，BoW，CNN，和LSTM的训练效果都要好于素贝叶斯和逻辑回归只用名字训练的模型。就这一节训练的四个模型而言，LSTM和CNN的效果最好（LSTM在全名上效果更显著），BOW次之，LR则相对较差。不过，由于任务过于简单，实际上这四个模型在效果上的差别并不是很明显。
